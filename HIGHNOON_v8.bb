;lisätty alkuun fullscreen -toggle
While Not KeyHit(1)
Cls
...
While (Not KeyHit(1)) ; päävalikkosilmukka alkaa
	Cls
...
Function rosvojen_muuvit()
		...
		For level.rosvo = Each rosvo
			...
			If (level\rosvo_liikkuu > 1) Then  
				...
				If level\moneybag = 1 Then DrawImage bag,level\rosvo_x-5,level\rosvo_y+15 ;jos rosvo kävi pankissa, piirretään rahasäkki
			EndIf
...
Function sheriffin_liike()
   ...
   ;alavasen
	...
	If (lentaa = 0) Then If (KeyDown(29)) Then
		...
		;oikea 2,3      vasen 0,1      ylös 4,5      		alas 6,7
		;yläoikea 8,9	ylävasen 10,11	alaoikea 14,15		alavasen 12,13		
...
For level.rosvo = Each rosvo 
	...
	If (level\rosvo_liikkuu = 1) Then ;tässä rosvo tehdään / regeneroituu, ellei niitä ole jo max-määrä ulkona
	...
	If (level\rosvo_liikkuu = 10) Then ; regeneroidaan viiveellä
		If (MilliSecs() > level\rosvo_ajastin2 + 3000) Then level\rosvo_liikkuu = 2
		level\rosvo_freimi_suunta = 0
		level\rosvo_y = Rnd(330,450) ;regeneroidessa täytyy antaa y-arvo uudestaan
	EndIf
...
If (MilliSecs() > level\rosvo_liike_ajastin + 100 And level\rosvo_liikkuu < 19) Then
	...
	Case 2 ; rosvo lähtee liikkeelle
			...
			EndIf
	 Case 3; pankkirosvo on mennyt juuri pankin tai saluunan ovesta, mitä sisällä tapahtuu?
		If (level\rosvo_suunta > 50) Then ; rosvo tulee oikealta
				...
				Else
					...
					;If level\rk > 400 Then 
					level\moneybag = 1 ;pankkirosvolle säkki käteen
				EndIf
		Else ; rosvo tulee vasemmalta
				If (level\rosvo_x < level\rk+80)Then 
					level\rosvo_x = level\rosvo_x + 3
				...
				EndIf
		EndIf  	
	Case 4	;kääntyy sisältä takaisinpäin
		If (level\rosvo_suunta > 50) Then ; rosvo tulee oikealta
				If (level\rosvo_x < level\rk) Then 
					level\rosvo_x = level\rosvo_x + 3
					level\rosvo_freimi_suunta = 2
		...
		Else ; rosvo tulee vasemmalta
				...
				EndIf
		EndIf
	Case 5 ;Bandit tulee ulos ja lähtee livohkaan
		If (level\rosvo_suunta > 50) Then ; rosvo tulee oikealta
				...
				Else
					level\rosvo_liikkuu = 18 ; rosvo pääsi livohkaan
					rosvoja_liikkeella = rosvoja_liikkeella - 1
					level\moneybag = 0 ; poistetaan säkki kädestä
					level\rosvo_y = Rnd(330,450)
				EndIf
		...
		Else ; rosvo tulee vasemmalta
				...
				Else
					level\rosvo_liikkuu = 18 ; rosvo pääsi livohkaan
					rosvoja_liikkeella = rosvoja_liikkeella - 1
					level\moneybag = 0 ;poistetaan säkki käsistä
					level\rosvo_y = Rnd(330,450)
				EndIf
		EndIf
	End Select
	...
	If (level\rosvo_y > 270 And level\rosvo_ampuu = 1 And MilliSecs() > level\rosvo_ampuu_ajastin + 50) Then
		...
		;mihin suuntaan rosvo kääntyy ampuessaan
...
; 		SetFont loparit
		...
		While ((alku <> creditlen) And (Not KeyHit(1)))
			...
			Cls	
				...
				If ((y+15) < -10) Then vah = 1;aloitetaan yläosasta vähentämään tekstejä
				y = y - 1 ;siirretään tekstiä ylös
				lisays = 0
				more = more + 1
				more2 = more2 + 1
